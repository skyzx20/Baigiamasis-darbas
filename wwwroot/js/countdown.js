function countDown(GameStartTime, GameID, GameButtonDisable, indexHolder) {
    ...
    var x = setInterval(function () {
        ...
        if (distance < 0) {
            ...
            var gameOver = document.getElementById(GameButtonDisable).innerHTML;
            if (gameOver == "Rezultatai") {
                var gameText = document.getElementById(GameID).innerHTML = "Rungtynės vyko:" + "<br />" + GameStartTime;
            }
            if (gameText == "Rungtynės žaidžiamos") {
                document.getElementById(GameButtonDisable).style.visibility = 'hidden';
            }
        }
    }, 1000);
}